...
-- Orders and pages the registrar's student list. It does NOT serve the search box: that asks
-- `name ILIKE '%termo%'`, and a leading wildcard is not something a btree can seek on. The scan is
-- invisible at 450 students per network and is left in place on purpose — it is the measurement
-- that has to justify Stage 13 (Section 5 of the stage document).
...
-- in the `Idempotency-Key` header, and this row is written by the FIRST statement of the write
-- transaction, not by the edge before it: the claim and the rows it refers to commit together, so
-- a rollback takes both and there is nothing to compensate. If the key already exists, the second
-- submission reprocesses nothing and goes back to `response_location`.
--
-- `response_location` is filled in after the commit, once the route has answered and the
-- destination is known. Losing that update loses a redirect, never a write.
--
