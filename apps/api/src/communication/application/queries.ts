...
import {
  ...
  countReads,
  sumReads,
} from '../infra/announcementReadRate';
import {
  countForGuardian,
  findForGuardian,
  listForGuardian,
  readAtForGuardian,
} from '../infra/announcementsOfGuardian';
...
export type BoardScope = (typeof BOARD_SCOPE)[keyof typeof BOARD_SCOPE];
...
export async function boardPage(
  ...
  const sql = reader();
  const filter =
    scope === BOARD_SCOPE.everything ? undefined : { read: scope === BOARD_SCOPE.read };
...
export async function announcementReadAt(
  networkId: string,
  userId: string,
  announcementId: string,
): Promise<string | null> {
  return await readAtForGuardian(reader(), networkId, userId, announcementId);
}
