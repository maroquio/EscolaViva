import { DEFAULT_PAGE_SIZE, MAX_PAGE } from './constants';

export { DEFAULT_PAGE_SIZE, MAX_PAGE };
...
export function requestedPage(raw: string | undefined | null): number {
  ...
  return withinBounds(number);
}

function withinBounds(page: number): number {
  return Math.min(MAX_PAGE, Math.max(1, Math.trunc(page) || 1));
}
...
export async function queryPage<T>(
  ...
  const requested = withinBounds(page);
