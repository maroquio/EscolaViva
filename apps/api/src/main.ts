...
async function shutdown(signal: string): Promise<void> {
  ...
  const jobsFinished = scheduler.stop();
...
process.on(SERVER.failureEvents.rejection, (reason) => {
  logger.error(redact({ reason: String(reason) }), PROCESS_MESSAGES.unhandledRejection);
});

process.on(SERVER.failureEvents.exception, (error: Error) => {
  logger.error(
    redact({ type: error.constructor.name, message: error.message, stack: error.stack }),
    PROCESS_MESSAGES.uncaughtException,
  );
});
