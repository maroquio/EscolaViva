import { reader, readerFresh } from '../../shared/db';
...
export async function validSession(sessionId: string): Promise<AuthenticatedUser | null> {
  ...
  const roles = await userRoleAssignments.userRoles(sql, network.id, user.id);
...
export async function isTeacherAtSchool(
  ...
  return await userRoleAssignments.isTeacherAtSchool(reader(), networkId, userId, schoolId);
}
...
export async function hasRoleInNetwork(
  ...
  if (!isUuid(networkId) || !isUuid(userId)) return false;
  return await userRoleAssignments.hasRoleInNetwork(reader(), networkId, userId, role);
}
...
export async function schoolTeachers(
  ...
  return await userRoleAssignments.schoolTeachers(reader(), networkId, schoolId);
}
