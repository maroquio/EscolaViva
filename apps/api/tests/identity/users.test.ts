 ...
 *
 * What identifies a person is the CPF, and it is unique PER NETWORK, not globally: two city halls
 * can carry the same registrar on record, and that is what makes it possible to sell the same
 * person to two accounts. The e-mail identifies nobody — it is contact, it is not typed at sign-in,
 * and it repeats on purpose, because a mother and a father share the family address.
 */
...
describe('inviteUser', () => {
  ...
  /*
   * The mother is registered, and then the father, with the address the family shares. Nothing in
   * `app_user` refuses it — there is no UNIQUE on `email`, and there is no check standing in for
   * one. This is the case the data model's comment cites as the reason the constraint does not
   * exist, so it is the case that has to run.
   */
  test('two people in the network share the family e-mail: the e-mail is contact, not identity', async () => {
    const network = await createNetwork();
    const school = await createTestSchool({ networkId: network.id });
    const familyAddress = 'familia.souza@provedor.br';
    await createUser({ networkId: network.id, name: 'Mãe da Ana', email: familyAddress });

    const father = await identity.inviteUser({
      networkId: network.id,
      name: 'Pai da Ana',
      email: familyAddress,
      cpf: generateCpf(6),
    ...
    expect(father.ok).toBe(true);
    const sharingTheAddress = (await identity.listUsers(network.id)).filter(
      (user) => user.email === familyAddress,
    );
    expect(sharingTheAddress.map((user) => user.name).sort()).toEqual(['Mãe da Ana', 'Pai da Ana']);
  });

  /*
   * The registrar double-clicks, or two people at two desks type the same transfer student in at
   * the same moment. Asking "is this CPF taken?" and then inserting leaves a gap where both answers
   * are "free"; the constraint is asked instead, so one insertion wins and the other comes back as
   * the refusal the person can read — not as a driver error carrying the CPF in its message.
   */
  test('two invitations racing on the same CPF: one is created, the other is refused', async () => {
    const network = await createNetwork();
    const school = await createTestSchool({ networkId: network.id });
    const sameCpf = generateCpf(11);
    const invitation = (name: string) =>
      identity.inviteUser({
        networkId: network.id,
        name,
        email: `${name.toLowerCase()}@escola.br`,
        cpf: sameCpf,
        roleAssignments: [{ schoolId: school.id, role: 'teacher' }],
      });

    const [first, second] = await Promise.all([invitation('Primeira'), invitation('Segunda')]);

    expect([first.ok, second.ok].sort()).toEqual([false, true]);
    const refused = first.ok ? second : first;
    expect(errorsOf(refused)).toEqual([
      { field: 'cpf', code: 'cpf_in_use', message: 'já existe usuário com este CPF na rede' },
    ]);
