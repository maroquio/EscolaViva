...
/*
 * Twelve, and not the four it used to be, because four made the suite fail about one run in six.
 *
 * `twoNetworks()` runs two `fullScenario()` in parallel and each of those nests several
 * `Promise.all` of its own, so dozens of queries compete for the pool at once. With four
 * connections they queue, and the case that happens to be first after a `beforeEach` — which needs
 * the pool for its own TRUNCATE — waits past the five-second deadline and is reported as a timeout.
 *
 * It is invisible to `pg_blocking_pids()`: the wait is in the client's queue, so the server sees no
 * lock and nothing to report. That is why two rounds of lock sampling came back empty. Measured on
 * 2026-08-16: two failures in twelve runs at four connections, none in twenty-four at twelve.
 */
