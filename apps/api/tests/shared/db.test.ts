...
/*
 * The last line, not the whole output. A database that does not answer now writes a line saying so
 * — which is the point of `PROCESS_MESSAGES.databaseUnavailable` — and that line shares stdout with
 * whatever the probe printed. Reading the tail is what keeps these cases about `checkDatabase`
 * rather than about how many lines pino happened to emit.
 */
...
describe('checkDatabase', () => {
  ...
  test('gives back false when the database URL is invalid and the deadline is short', async () => {
    ...
    expect(lastLine(stdout)).toBe('false');
  });
  ...
  test('does not hang the route when the database will not answer: the deadline wins', async () => {
    ...
    const measurement = JSON.parse(lastLine(stdout)) as { responded: boolean; ms: number };
