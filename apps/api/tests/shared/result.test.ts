...
describe('fieldFailure', () => {
  test('produces a single error tied to the form field', () => {
    ...
    const result = fieldFailure(field, 'cpf_in_use', 'já existe usuário com este CPF na rede');

    expect(result).toEqual({
      ok: false,
      errors: [{ field: 'cpf', code: 'cpf_in_use', message: 'já existe usuário com este CPF na rede' }],
    });
  });
});
