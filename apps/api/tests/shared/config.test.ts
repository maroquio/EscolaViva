...
/*
 * The two variables a separately published front would need. Both are born empty, and empty is the
 * deployment this stage actually runs: one origin, and a host-only cookie. The cases below pin that
 * default, because a variable that silently acquires a value is how a same-origin deployment starts
 * emitting CORS headers nobody asked for.
 */
describe('loadConfig — ALLOWED_ORIGINS and COOKIE_DOMAIN', () => {
  ...
  test('the cookie domain is null when it is not declared', () => {
    const withoutDomain = loadConfig({ ...REQUIRED });
    const withDomain = loadConfig({ ...REQUIRED, COOKIE_DOMAIN: '.escolaviva.test' });

    expect(withoutDomain.cookieDomain).toBeNull();
    expect(withDomain.cookieDomain).toBe('.escolaviva.test');
  });

  /*
   * `loadConfig` runs `withoutEmptyValues` before parsing, so `ALLOWED_ORIGINS=` in a `.env` reaches
   * the schema as `undefined` rather than as `''`. Only the schema default rescues it, which is why
   * the blank form is a case of its own and not a repetition of the two above.
   */
