...
describe('linkGuardian', () => {
  test('ties the guardian to the student under the relationship given', async () => {
    ...
    const guardian = await createGuardian({
      networkId: network.id, schoolId: school.id, name: 'Carla Dias', email: 'carla@familia.br',
    });
  ...
  test('refuses the same link twice', async () => {
    ...
    const school = await createSchool({ networkId: network.id });
    const student = await createStudent({ networkId: network.id });
    const guardian = await createGuardian({ networkId: network.id, schoolId: school.id });
  ...
  /*
   * Being reachable in the network is not the same as answering for a child.
   *
   * The guardian's whole view — report card, attendance, board — is authorised by one row in
   * `student_guardian`, and nothing downstream ever consults `user_role` again. So this is the gate:
   * whoever gets written here can read that child's record. It used to accept any uuid belonging to
   * the network, which meant the teacher, the network administrator and the registrar themselves
   * could be tied to a student and would then see the record of a child they answer for in no way.
   */
  test('refuses somebody who does not hold the guardian role, whatever else they hold', async () => {
    const network = await createNetwork();
    const school = await createSchool({ networkId: network.id });
    const student = await createStudent({ networkId: network.id });
    ...
    expect(errorsOf(result)[0]).toMatchObject({ field: 'userId', code: 'not_a_guardian' });
    expect(await academics.studentGuardians(network.id, student.id)).toHaveLength(0);
  });
  ...
  test('refuses a student from another network', async () => {
    ...
    const guardian = await createGuardian({ networkId: ours.id, schoolId: ourSchool.id });
