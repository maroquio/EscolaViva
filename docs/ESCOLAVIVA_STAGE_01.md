...
- **Code:** a modular monolith with four domains (`identity`, `academics`, `assessment`,
  `communication`), server-rendered HTML, a session in a table with the cookie carrying only its
...
├─ tests/                         # one folder per module + web/, support/ and the frozen screens
├─ scripts/
...
│  ├─ golden.ts                  # compares every screen against its frozen HTML
│  ├─ magic-values.ts            # refuses a loose literal in src/
│  ├─ build-assets.ts            # produces app.<hash>.css  (I10)
│  ├─ backup.sh
...
-- Academics: the registrar searching, filtering, and reaching the children of a guardian.
...
|------|-----------|--------------|
| Registrar | registers students and guardians, links guardian to student, enrols, transfers, registers class groups and subjects | session |
