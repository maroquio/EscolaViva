...
export const E2E_CLEAN_MESSAGE = (counts: {
  alunos: number;
  usuarios: number;
  escolas: number;
  fechamentos: number;
  ...
  `removidos: ${counts.alunos} aluno(s), ${counts.usuarios} usuário(s), ` +
  `${counts.escolas} escola(s), ${counts.fechamentos} fechamento(s), ` +
  `${counts.chamadas} chamada(s), ${counts.leituras} leitura(s)`;
