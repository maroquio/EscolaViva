...
const INDEXED_FILES = /(?:^|\/)constants\.ts$/;

const THIS_FILE = 'scripts/magic-values.ts';

const HTTP_STATUSES = new Set([
  200, 201, 202, 204, 300, 301, 302, 303, 304, 400, 401, 403, 404, 405, 409, 410, 415, 422, 429,
  500, 502, 503, 504,
]);
...
const jsxText = (node: ts.Node): string | undefined => {
  if (!ts.isJsxText(node)) return undefined;
...
const valueKey = (node: ts.Node): string | undefined => {
  ...
  const shown = jsxText(node);
  if (shown !== undefined) return textKey(shown);
...
const withoutJsxContainer = (node: ts.Node): ts.Node => {
  const bare = withoutWrapper(node);
  const parent = bare.parent;
  return parent !== undefined && ts.isJsxExpression(parent) ? withoutWrapper(parent) : bare;
};

const isShownWhereItIsWritten = (node: ts.Node): boolean => {
  const parent = withoutJsxContainer(node).parent;
...
const isSentence = (node: ts.Node): boolean =>
  ...
  TWO_WORDS_IN_A_ROW.test(jsxText(node) ?? '');

const isScreenWording = (node: ts.Node): boolean => {
  const text = literalText(node);
  return text !== undefined && ONE_WORD.test(text) && !IDENTIFIER.test(text);
};
...
const duplicateOwner = (node: ts.Node, context: Context, file: string): Owner | undefined => {
  const key = valueKey(node);
  if (key === undefined) return undefined;
  const owners = (indexByValue.get(key) ?? []).filter((owner) => owner.file !== file);
  const copiesAnOwnedWording = !context.declaration && isScreenWording(node);
  return pickOwner(
    owners,
    context.path,
    isRegularExpression(node) || isSentence(node) || copiesAnOwnedWording,
  );
};
...
const pickOwner = (
  ...
  path: string,
  worthAConstantAnywhere: boolean,
): Owner | undefined => {
  ...
  return worthAConstantAnywhere ? owners[0] : undefined;
};
...
const PRODUCT_MODULES: readonly string[] = [
  'apps/api/src/',
  'apps/web/src/',
...
const isProductCode = (file: string): boolean =>
  PRODUCT_MODULES.some((module) => file.startsWith(module));

const CONTRACTS_DIRECTORY = 'packages/contracts/src/';
...
function analyzeTypeScript(file: string, source: string): Finding[] {
  ...
  const count = (node: ts.Node, text: string): void => {
    if (!isProductCode(file) || !isCountableText(text)) return;
    const { line, character } = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
  ...
  const copyReason = (node: ts.Node, context: Context): string | undefined => {
    if (
      (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) &&
    ...
    const owner = duplicateOwner(node, context, file);
  ...
  const check = (node: ts.Node, context: Context): void => {
    if (ts.isJsxText(node)) {
      ...
      count(node, shown);
      const copiedText = copyReason(node, context);
      if (copiedText !== undefined) record(node, copiedText);
      return;
    }
    if (
      (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) &&
    ...
    const copy = copyReason(node, context);
    if (copy !== undefined) {
      record(node, copy);
      return;
    }
    if (context.declaration || isNamedValue(node) || isShownWhereItIsWritten(node)) return;
...
for (const file of targets) {
  ...
  findings.push(...analyzeTypeScript(file, source));
}
...
for (const entry of liveSilenced) {
  ...
  if (file.startsWith(CONTRACTS_DIRECTORY)) continue;
